...
function createFactory( prng, dtypes, dtype ) {
	var isValidDataType;
	if ( !isFunction( prng ) ) {
		throw new TypeError( format( '1oH3c', prng ) );
	}
	if ( !isMethodIn( prng, 'factory' ) ) {
		throw new TypeError( format( '1oHFM', 'factory' ) );
	}
	...
	if ( !isStringArray( dtypes ) ) {
		throw new TypeError( format( '1oHFN', dtypes ) );
	}
	...
	if ( !contains( dtypes, dtype ) ) {
		throw new TypeError( format( '1oHFO', dtype ) );
	}
	...
	function factory() {
		...
		if ( hasOwnProp( opts, 'dtype' ) ) {
			...
			if ( !isValidDataType( dt ) ) {
				throw new TypeError( format( '1oH4S', 'dtype', dtypes.join( '", "' ), dt ) );
			}
		} else {
			dt = dtype;
		}
